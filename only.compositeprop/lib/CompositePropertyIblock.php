...
class CompositePropertyIblock
{
    ...
    public static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        foreach ($arFields as $code => $arItem){
            ...
            else if ($arItem['TYPE'] === 'html-editor'){
    ...
    public static function ConvertFromDB($arProperty, $arValue)
    {
        ...
        if (!empty($arValue['VALUE'])) {
            ...
            elseif (is_array($arValue['VALUE'])) {
                $return['VALUE'] = $arValue['VALUE'];
            }
        }
    ...
    public static function showHtmlEditor($code, $title, $arValue, $strHTMLControlName)
    {
        ...
        $v = !empty($arValue['VALUE'][$code]) ? $arValue['VALUE'][$code] : '';
        $incorrectName = preg_replace('/[^a-zA-Z0-9]/', '', $strHTMLControlName['VALUE'] . '[' . $code . ']');
        ob_start();
		...
		ob_end_clean();
        $html = str_replace('name="'. $incorrectName . '"', 'name="' . $strHTMLControlName['VALUE'] . '[' . $code . ']"', $html);
    ...
    private static function getOptionList($selected = 'string')
    {
        ...
        $arOption = [
            ...
            'html-editor' => Loc::getMessage('IEX_CPROP_FIELD_TYPE_HTML_EDITOR'),
        ];
